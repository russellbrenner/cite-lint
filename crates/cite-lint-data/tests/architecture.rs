...
/// Parse the `cite-lint-*` path dependencies out of a crate manifest.
/// (Line-based on purpose: manifests are ours and simple.)
fn cite_lint_deps(manifest: &str) -> Vec<String> {
...
/// data → core → host → cite-lint(SDK) → {cli, lsp}.
#[test]
fn dependency_graph_is_one_way() {
    let expected: BTreeMap<&str, Vec<&str>> = BTreeMap::from([
        ("cite-lint-data", vec![]),
        ("cite-lint-core", vec!["cite-lint-data"]),
        ("cite-lint-host", vec!["cite-lint-core"]),
        (
            "cite-lint",
            vec!["cite-lint-core", "cite-lint-data", "cite-lint-host"],
        ),
        ("cite-lint-cli", vec!["cite-lint"]),
        ("cite-lint-lsp", vec!["cite-lint"]),
    ]);
    ...
    for (krate, allowed) in &expected {
        ...
        let actual = cite_lint_deps(&manifest);
        assert_eq!(
...
/// string-literalled in `cite-lint-core` — rules read vocab from data.
...
fn core_never_inlines_controlled_vocabulary() {
    let tables = cite_lint_data::load("aglc4").expect("edition loads");
    ...
    let src_dir = workspace_root().join("crates/cite-lint-core/src");
        ...
        for term in &vocab {
            assert!(
                ...
                "{} inlines controlled vocab {} — read it from \
                 cite-lint-data instead (invariant 2)",
                path.display(),
                term
            );
        }
...
fn lint_path_has_no_io_dependencies() {
    let root = workspace_root();
    for krate in ["cite-lint-data", "cite-lint-core", "cite-lint-host"] {
        let manifest = fs::read_to_string(root.join("crates").join(krate).join("Cargo.toml"))
            .expect("manifest");
        let mut in_deps = false;
        for line in manifest.lines() {
