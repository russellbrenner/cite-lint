...
//! document. Severity/confidence are re-exported from `cite-lint-data` so the
//! IR and the diagnostics share one vocabulary.
//! Depends on: `cite-lint-data` only.

pub use cite_lint_data::{AglcRef, Confidence, Severity};
