//! `cite-lint-lsp` — the LSP surface (plan 06 T3).
//!
...
fn main() -> ExitCode {
    ...
    let edition_count = cite_lint::Session::new("aglc4")
        .and_then(|s| s.editions())
        .map(|e| e.len())
        .unwrap_or(0);
    eprintln!(
        "cite-lint-lsp: not yet implemented (M1 stub; {edition_count} edition(s) loaded). \
