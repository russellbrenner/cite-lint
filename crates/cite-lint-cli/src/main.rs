//! `cite-lint` — the batch CLI surface (plan 06).
//!
//! A thin shell over the `cite-lint` SDK: parse args → call a capability →
...
use cite_lint::{Diagnostic, HostKind, Session};

const USAGE: &str = "cite-lint — AGLC4 citation linter

USAGE:
    cite-lint check [PATH ...|-] [--format text|json] [--edition ID] [--host markdown|plain]
    cite-lint parse <CITATION>
    cite-lint explain <CODE>|--all
    cite-lint fix <PATH> [--edition ID] [--host markdown|plain]
    cite-lint editions
    cite-lint --help | --version
...
fn run(args: Vec<String>) -> Result<ExitCode, String> {
    ...
    match command.as_str() {
        ...
        "--version" => {
            println!("cite-lint {}", env!("CARGO_PKG_VERSION"));
            Ok(ExitCode::SUCCESS)
        }
...
fn read_input(path: &str) -> Result<(String, String, HostKind), String> {
    if path == "-" {
        ...
        Ok((path.to_string(), text, cite_lint::host_for_path(path)))
    }
}
...
fn cmd_check(rest: &[String]) -> Result<ExitCode, String> {
    ...
    match flags.format.as_str() {
        ...
        _ => {
            ...
            eprintln!(
                "cite-lint: {} diagnostic(s) across {} input(s)",
...
fn cmd_fix(rest: &[String]) -> Result<ExitCode, String> {
    ...
    print!("{}", result.fixed);
    eprintln!("cite-lint: applied {} fix(es)", result.applied);
    Ok(ExitCode::SUCCESS)
}
